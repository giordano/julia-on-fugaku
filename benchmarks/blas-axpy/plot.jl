...
function plot_benchmarks(title, julia, fujitsu, blis, armpl)
...
function plot_benchmarks_no_armpl(title, julia, fujitsu, blis)
    p = plot_benchmarks(title, julia)
    plot!(p, fujitsu[:, 1], gflops.(fujitsu[:, 1], fujitsu[:, 2]); label="Fujitsu BLAS", marker=:auto, markersize=3)
    plot!(p, blis[:, 1], gflops.(blis[:, 1], blis[:, 2]); label="BLIS", marker=:auto, markersize=3)
    return p
...
plot_benchmarks("axpy (single precision)", julia_32, fujitsu_32, blis_32, armpl_32)
savefig("axpy-single.pdf")
plot_benchmarks("axpy (double precision)", julia_64, fujitsu_64, blis_64, armpl_64)
